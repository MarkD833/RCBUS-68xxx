...
*
*******************************************************************************
* This is my own simple monitor program for my RCBus 68000 board. It supports
* simple S-Record loading as well as memory display & simple memory editing.
* In addition it allows reading and writing bytes to the RCBus IO space.
*
* There's no breakpoints, single stepping or disassembly stuff.
*******************************************************************************
* Hardware assumptions:
*
* + My RCBus SIO board is used and an MC68681 (or equivalent) SIO chip is
*   present at the address DUART1 (see devices.inc)
* + ROM is present at address 0x000000 onwards
* + RAM is present at address RAM_BASE (see memory.inc)
*******************************************************************************
* EEPROM programming:
*
...
* Exception Handling:
*
* ROM starts at 0x000000 and holds the ROM exception vector table that points
* to the exception handling routines in ROM. These ROM handlers fetch the
* addresses of the actual exception handling routines from the start of RAM 
* or wherever VEC_BASE is defined and re-directs execution to that address.
...
* EhBASIC Support:
*
* The EhBASIC s-record file can be programmed into the EEPROMs at the same
* time as this monitor and should reside at address EHBASIC_BASE (see memory.inc).
*******************************************************************************
* Interrupts:
*
* An interrupt handler is setup to handle autovector level 2 interrupts from
* the DUART. Make sure that the jumper is set on the SIO board for level 2
* interrupts.
*******************************************************************************
* CP/M-68K Support
* V1.3 of CP/M-68K is supported. The CPM400 s-record file should be
...
*******************************************************************************
* The serial port originally ran at 38400 baud for the MC68681 SIO chip.
* Alternative compatibe chips were made by Philips (SCC68681,SCN68681 & SCC68692)
* that supported higher baud rates using a special test mode.
* Set HI_SPEED to 1 to enable 115200 baud or 0 for standard 38400 baud.  
*******************************************************************************
HI_SPEED	equ	0

*******************************************************************************
* ramVecJump : Macro to "jump" to exception handler in the RAM vector table
...
SERIAL_BUFFER_UPPER equ 200
SERIAL_BUFFER_LOWER equ 25
...
*                                                                                       
*******************************************************************************
* 68000 ROM Exception Vector Table
*
* All exceptions eventually get the handler addresses from the RAM vector table. 
*******************************************************************************
    ...
    DC.L    jmpBusErr      *02 Bus Error
    DC.L    jmpAddrErr     *03 Address Error
    ...
    DC.L    jmpTrap15      *2E TRAP #15 Instruction Vector
...
*******************************************************************************
* Place the monitor program after the CP/M BIOS code.
*******************************************************************************
...
*                                                                                       
*******************************************************************************
* 68000 RAM Exception Vector Table
*
* This is the initial exception vector table that gets copied into RAM.
* It initially redirects back to the basic exception handlers in ROM until the
* user program inserts their own exception handler address.
*******************************************************************************
...
ramIVT:
    ...
    DC.L    serISR         *1A Level 2 Interrupt Autovector
...
*******************************************************************************
* Below are the actual exception handlers that the RAM based vector table
* initially points to. The user can overwrite the RAM vector table addresses
* with the address of their own exception handler. These basic handlers simply
* write a message to the serial port and enter an endless loop.
*******************************************************************************
...
*------------------------------------------------------------------------------
handleBusErr:
    lea     bemsg1(PC), a0	    * A0 = first message address
    bsr.w   putString           * display it
	move.l	10(a7),d0		    * get the program counter 
	bsr.w	writeAddr32         * display it
	
    lea     bemsg2(PC), a0	    * A0 = second message address
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleAddrErr:
    lea     aemsg1(PC), a0	    * A0 = first message address
    bsr.w   putString           * display it
	move.l	10(a7),d0		    * get the program counter 
	bsr.w	writeAddr32         * display it
	
    lea     aemsg2(PC), a0	    * A0 = second message address
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleIllInst:
    lea     iimsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleZeroDiv:
    lea     zdmsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleChkInst:
    lea     cimsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrapV:
    lea     tvmsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handlePriv:
    lea     pvmsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrace:
    lea     trmsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleLineA:
    lea     lamsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleLineF:
    lea     lfmsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
handleRes10:
handleRes11:
    lea     rimsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleUnInit:
    lea     uimsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleSpur:
    lea     simsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleAV1:
    lea     av1msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleAV2:
    lea     av2msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleAV3:
    lea     av3msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleAV4:
    lea     av4msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleAV5:
    lea     av5msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleAV6:
    lea     av6msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleAV7:
    lea     av7msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap0:
    lea     tr0msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap1:
    lea     tr1msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap2:
    lea     tr2msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap3:
    lea     tr3msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap4:
    lea     tr4msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap5:
    lea     tr5msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap6:
    lea     tr6msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap7:
    lea     tr7msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap8:
    lea     tr8msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap9:
    lea     tr9msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap10:
    lea     tr10msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap11:
    lea     tr11msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap12:
    lea     tr12msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap13:
    lea     tr13msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap14:
    lea     tr14msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleTrap15:
    lea     tr15msg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
...
*------------------------------------------------------------------------------
handleUnused:
    lea     usermsg(PC), a0	    * A0 = address of message to display
	bra.w	handleCleanUp
	
*------------------------------------------------------------------------------
* All exception handlers jump here to output the PC of the next instruction
* after the exception occured.
* A0 holds the address of a null terminated message to be displayed first.
*------------------------------------------------------------------------------
handleCleanUp:
    bsr.w   putString           * print out the relevant message
	move.l	2(a7),d0		    * get the saved PC address
	bsr.w 	writeAddr32         * and display it
	bsr.w 	putCRLF
.infloop:
    bra.s   .infloop            * and stay here forever ...
...
*                                                                               
*******************************************************************************
* Monitor execution starts here after power-on or a reset.
*******************************************************************************
...
START:
	...
	* copy the RAM exception vector table into RAM
	move.l	#ramIVT,a0		    * start of RAM exception table in ROM
...
.copy:
    ...
    bsr.w   serInit             * initialise serial port
...
*
* Assumes that there is a 68681 DUART at the address defined by DUART1 in
* devices.inc and initialises port A and enables the rx interrupt.
*------------------------------------------------------------------------------
serInit:
    ...
    move.l  #0,ser0RxHead       * Set queue head and tail ptrs to 0
    move.w  #0,ser0RxSize       * No chars in receive queue

    move.b  #$00, ACR           * Baud Rate Set #1
    move.b  #$13, MRA           * No Parity & 8-bit
    move.b  #$07, MRA           * Normal Mode, No CTS/RTS & 1 stop bit

	* if HI_SPEED = 0 then set 38400 baud
	ifeq HI_SPEED
    move.b  #BAUD_38400, CSRA   * Set Tx and Rx baud rates to 38400
	endc

	* if HI_SPEED > 0 then set 115200 baud
	ifgt HI_SPEED
    move.b  CRA,d0              * Read CRA - sets BRG test mode
    move.b  #BAUD_1200, CSRA    * Set Tx and Rx baud rates to 115200 (old 1200)
	endc
	
    move.b  #$02, IMR           * Rx interrupt enabled
    move.b  #$05, CRA           * Enable Transmit/Recieve
    move.b  #$01, SOPR          * set OP0 low - i.e. CTS low
    rts

*------------------------------------------------------------------------------
* Serial port interrupt handler
*
* Should only be called when a char has been received.
*
*------------------------------------------------------------------------------
serISR:
    * save registers we are about to trash
    movem.l d0-d2/a0,-(sp)

    btst.b  #1,ISR              * check the RXRDYA bit
    beq.w   serExit

    * a char has been received so save it in the rx queue
    move.b  RBA,d0              * get the character
    movea.l #ser0RxQueue,a0     * A0 = ptr to rx queue parameters
    move.l  (a0),d1             * D1.L MSW = head and LSW = tail
    swap    d1                  * D1.L MSW = tail and LSW = head
    move.w  d1,d2               * D2.W holds the queue head index
    swap    d1                  * D1.L MSW = head and LSW = tail


    addq.w  #1,d2               * advance the head index
    andi.w  #SERIAL_BUFFER_MASK,d2
    cmp.w   d1,d2               * if head = tail then buffer full
    bne.s   serSave             * if head <> tail then add char to buffer

    * the serial buffer is full - what now?
    bra.w   serExit

serSave:
    swap    d1                  * D1.L MSW = tail and LSW = head
    andi.l  #$0000FFFF,d1       * D1 holds just the old head index
    move.b  d0,6(a0,d1)         * save the char to the rx queue
    move.w  d2,(a0)             * save the updated head index
    addq.w  #1,ser0RxSize       * increment number of chars in rx queue

    * check if rx queue is nearly full
    cmpi.w  #SERIAL_BUFFER_UPPER,ser0RxSize
    ble.s   serExit

    * no of chars in queue has reached the upper threshold
    * set CTS HIGH to hopefully tell the host to stop sending.
    move.b  #$01, ROPR          * set OP0 high
...
*------------------------------------------------------------------------------
* Write a character directly to UART Port A, blocking if UART is not ready
*
* D0 = char to send
*------------------------------------------------------------------------------
putc:
...
*------------------------------------------------------------------------------
* Read a character from the rx queue - blocking call so will wait for character
*
* D0 = recevied char
*------------------------------------------------------------------------------
getc:
    cmpi.w  #0,ser0RxSize       * check number of chars in rx queue
	beq.s	getc                * keep checking till there is a char available

    * save registers we are about to trash
    movem.l d1/a0,-(sp)
    
    * get the character from the tail of the queue
    movea.l #ser0RxQueue,a0     * A0 = ptr to rx queue parameters
    move.l  (a0),d1             * D1.L MSW = head and LSW = tail
    move.b  6(a0,d1.w),d0       * read the char from the queue tail
    addq.w  #1,d1               * advance the tail index
    andi.w  #SERIAL_BUFFER_MASK,d1
    move.w  d1,2(a0)            * save the updated tail index
    subq.w  #1,ser0RxSize       * decrement number of chars in rx queue

    * check if rx queue is nearly empty
    cmpi.w  #SERIAL_BUFFER_LOWER,ser0RxSize
    bge.s   getExit

    * no of chars in queue has reached the lower threshold
    * set CTS LOW to hopefully tell the host to start sending.
    move.b  #$01, SOPR          * set OP0 low
...
*------------------------------------------------------------------------------
* These are the EASy68K tasks that are not yet implemented
* Simply print out a message showing the task number and stop in an endless loop.
*------------------------------------------------------------------------------
...
.wn1:
    ...
    bsr.w   putc                * write the ASCII digit out
    move.w  (sp)+,d0            * restore D0
    rts
...
strBanner1:
	dc.b 10,13
	dc.b 'Simple RCBus 68000 ROM Monitor v1.5',10,13
...
*
*******************************************************************************
* Private monitor variables go here in a block of 2048 bytes (defined by MON_PRIV)
* and are located at the top of RAM (RAM_BASE+RAM_LEN-MON_PRIV).
*******************************************************************************

	ORG		STACK_START

* keep these serial rx queue variables in this order
ser0RxQueue:
ser0RxHead: ds.w    1
ser0RxTail: ds.w    1
ser0RxSize: ds.w    1
ser0RxBuf:  ds.b    SERIAL_BUFFER_SIZE
